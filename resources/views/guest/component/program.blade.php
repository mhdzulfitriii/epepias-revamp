<div class="">
    <!-- Card Blog -->
    <div class="max-w-[85rem] px-4 py-10 sm:px-6 lg:px-8 lg:py-14 mx-auto">
        <!-- Title -->
        <div class="max-w-2xl mx-auto text-center mb-10 lg:mb-14">
            <h2 class="text-2xl font-bold md:text-4xl md:leading-tight">Program & Usrah Terkini PEPIAS</h2>
            <p class="mt-1 text-gray-600">See how game-changing companies are making the most of every engagement with
                Preline.</p>
        </div>
        <!-- End Title -->

        <!-- Grid -->
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-6">
            <div
                class="w-full border bg-white rounded-2xl min-h-[20vh]  max-h-[70vh] overflow-y-auto
  [&::-webkit-scrollbar]:w-2  
  [&::-webkit-scrollbar-track]:rounded-full
  [&::-webkit-scrollbar-track]:bg-gray-100
  [&::-webkit-scrollbar-thumb]:rounded-full
  [&::-webkit-scrollbar-thumb]:bg-gray-200">
                <!-- Card -->
                @foreach ($program as $item)
                    <a class="block border border-gray-200 rounded-lg hover:shadow-sm focus:outline-none m-4"
                        href="{{route('guest.view.program', ['slug'=>$item->Slug])}}">
                        <div class="relative flex items-center overflow-hidden">
                            <img class="w-[100px] sm:w-48 h-full absolute inset-0 object-cover rounded-s-lg"
                                src="{{ asset($item->image->link) }}" alt="Blog Image">

                            <div class="grow p-4 ms-32 sm:ms-48">
                                <div class="min-h-[100px] flex flex-col justify-center">

                                    <h3 class="font-semibold text-sm text-gray-800">
                                        {{ $item->NamaProgram }}
                                    </h3>
                                    <p class="mt-1 text-sm text-gray-500">
                                        Program {{ $item->Persatuan->Singkatan }} dan bertempat di {{ $item->Tempat }}
                                        pada
                                        {{ \Carbon\Carbon::parse($item->StartDate)->translatedFormat('j F Y') }}
                                        @if ($item->EndDate != null && $item->EndDate != $item->StartDate)
                                            hingga
                                            {{ \Carbon\Carbon::parse($item->EndDate)->translatedFormat('j F Y') }}
                                        @endif

                                    </p>

                                    <div class=" flex gap-3 mt-3">
                                        <div
                                            class="my-1 text-xs text-black border bg-lime-200 w-fit py-0.5 px-2 rounded-lg">
                                            {{ $item->Mode }}
                                        </div>

                                        <div
                                            class="my-1 text-xs text-black border bg-lime-200 w-fit py-0.5 px-2 rounded-lg">
                                            {{ $item->JenisProgram }}
                                        </div>
                                    </div>

                                </div>
                            </div>
                        </div>
                    </a>
                @endforeach


            </div>

            <div
                class="w-full border bg-white rounded-2xl min-h-[20vh]  max-h-[70vh] overflow-y-auto
  [&::-webkit-scrollbar]:w-2  
  [&::-webkit-scrollbar-track]:rounded-full
  [&::-webkit-scrollbar-track]:bg-gray-100
  [&::-webkit-scrollbar-thumb]:rounded-full
  [&::-webkit-scrollbar-thumb]:bg-gray-200">
                <!-- Card -->
                <a class="block border border-gray-200 rounded-lg hover:shadow-sm focus:outline-none m-4"
                    href="#">
                    <div class="relative flex items-center overflow-hidden">
                        <img class="w-32 sm:w-48 h-full absolute inset-0 object-cover rounded-s-lg"
                            src="https://images.unsplash.com/photo-1661956600655-e772b2b97db4?q=80&w=560&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D"
                            alt="Blog Image">

                        <div class="grow p-4 ms-32 sm:ms-48">
                            <div class="min-h-24 flex flex-col justify-center">
                                <h3 class="font-semibold text-sm text-gray-800">
                                    Studio by Mailchimp
                                </h3>
                                <p class="mt-1 text-sm text-gray-500">
                                    Produce professional, reliable streams easily leveraging Mailchimp's innovative
                                    broadcast studio.
                                </p>
                            </div>
                        </div>
                    </div>
                </a>

                <a class="block border border-gray-200 rounded-lg hover:shadow-sm focus:outline-none m-4"
                    href="#">
                    <div class="relative flex items-center overflow-hidden">
                        <img class="w-32 sm:w-48 h-full absolute inset-0 object-cover rounded-s-lg"
                            src="https://images.unsplash.com/photo-1661956600655-e772b2b97db4?q=80&w=560&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D"
                            alt="Blog Image">

                        <div class="grow p-4 ms-32 sm:ms-48">
                            <div class="min-h-24 flex flex-col justify-center">
                                <h3 class="font-semibold text-sm text-gray-800">
                                    Studio by Mailchimp
                                </h3>
                                <p class="mt-1 text-sm text-gray-500">
                                    Produce professional, reliable streams easily leveraging Mailchimp's innovative
                                    broadcast studio.
                                </p>
                            </div>
                        </div>
                    </div>
                </a>

                <a class="block border border-gray-200 rounded-lg hover:shadow-sm focus:outline-none m-4"
                    href="#">
                    <div class="relative flex items-center overflow-hidden">
                        <img class="w-32 sm:w-48 h-full absolute inset-0 object-cover rounded-s-lg"
                            src="https://images.unsplash.com/photo-1661956600655-e772b2b97db4?q=80&w=560&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D"
                            alt="Blog Image">

                        <div class="grow p-4 ms-32 sm:ms-48">
                            <div class="min-h-24 flex flex-col justify-center">
                                <h3 class="font-semibold text-sm text-gray-800">
                                    Studio by Mailchimp
                                </h3>
                                <p class="mt-1 text-sm text-gray-500">
                                    Produce professional, reliable streams easily leveraging Mailchimp's innovative
                                    broadcast studio.
                                </p>
                            </div>
                        </div>
                    </div>
                </a>

                <a class="block border border-gray-200 rounded-lg hover:shadow-sm focus:outline-none m-4"
                    href="#">
                    <div class="relative flex items-center overflow-hidden">
                        <img class="w-32 sm:w-48 h-full absolute inset-0 object-cover rounded-s-lg"
                            src="https://images.unsplash.com/photo-1661956600655-e772b2b97db4?q=80&w=560&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D"
                            alt="Blog Image">

                        <div class="grow p-4 ms-32 sm:ms-48">
                            <div class="min-h-24 flex flex-col justify-center">
                                <h3 class="font-semibold text-sm text-gray-800">
                                    Studio by Mailchimp
                                </h3>
                                <p class="mt-1 text-sm text-gray-500">
                                    Produce professional, reliable streams easily leveraging Mailchimp's innovative
                                    broadcast studio.
                                </p>
                            </div>
                        </div>
                    </div>
                </a>
                <!-- End Card -->
            </div>
        </div>
        <!-- End Grid -->
    </div>
    <!-- End Card Blog -->
</div>
